...
function CompanyDirectory() {
  ...
  const [scoreRange, setScoreRange] = useState([0, 14]);
  ...
  const filteredAndSortedCompanies = useMemo(() => {
    let filtered = companies.filter(company => {
      ...
      const matchesTopPerformers = !showTopPerformers || company.score >= 12;
  ...
  const hasActiveFilters = searchTerm || selectedIndustry || scoreRange[0] > 0 || scoreRange[1] < 14 || sortBy !== 'name' || showTopPerformers;
